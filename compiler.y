...
if_stat : IFSYM LPAREN expression RPAREN code_address { genCode(jpc, 0, 0); }
statement else_stat {
  // 如果 if 条件不成立, 直接跳转
  code[$5].a = $8;
}

else_stat : ELSESYM code_address { genCode(jmp, 0, 0); }
statement {
...
while_stat : WHILESYM LPAREN code_address expression RPAREN code_address {
  genCode(jpc, 0, 0);
}
statement {
  genCode(jmp, 0, $3);
...
write_stat : WRITESYM { is_write = true; }
expression_stat;

read_stat : READSYM var SEMICOLON {
  if (is_char)
    genCode(opr, 0, 20);  // 读 char
  else
    genCode(opr, 0, 16);  // 读 int, bool

  if (is_array_element)
    genCode(opr, 0, 17);  // 存到数组
  else
...
simple_expr : additive_expr | additive_expr GRT additive_expr {
  genCode(opr, 0, 12);
}
| additive_expr LES additive_expr { genCode(opr, 0, 10); }
| additive_expr GEQ additive_expr { genCode(opr, 0, 11); }
| additive_expr LEQ additive_expr { genCode(opr, 0, 13); }
...
| NOT additive_expr { genCode(opr, 0, 25); };

term : factor | term TIMES factor { genCode(opr, 0, 4); }
| term DIVIDE factor { genCode(opr, 0, 5); }
| term MOD factor { genCode(opr, 0, 21); };

factor : LPAREN expression RPAREN | var {
  if (is_array_element)
    genCode(opr, 0, 18);
  else
...
| NUMBER { genCode(lit, 0, $1); }
| TRUESYM { genCode(lit, 0, 1); }
| FALSESYM { genCode(lit, 0, 0); };
...
void interpret(GtkWidget* frame) {
  ...
  do {
    ...
    switch (i.op) {
      ...
      case opr:  // 数学、逻辑运算
        switch (i.a) {
          ...
          case 14:  // 栈顶值输出
            ...
            gtk_text_buffer_insert(GTK_TEXT_BUFFER(buffer), &end, out,
                                   strlen(out));
            break;
          case 15:  // 输出换行符
            sprintf(out, "\n");
          ...
          case 16:  // 读入一个输入置于栈顶
            ...
            GtkWidget* input;
            GtkWidget* table;
            dialog = gtk_dialog_new_with_buttons("Input", NULL,
          ...
          case 20:  // 读入一个字符置于栈顶
            ...
            char_dialog = gtk_dialog_new_with_buttons(
                "Input", NULL, GTK_DIALOG_MODAL, GTK_STOCK_OK, GTK_RESPONSE_OK,
                NULL);
            gtk_dialog_set_default_response(GTK_DIALOG(char_dialog),
                                            GTK_RESPONSE_OK);

            char_title = gtk_label_new("input");
            char_input = gtk_entry_new();
            char_table = gtk_table_new(2, 2, FALSE);
            gtk_table_attach_defaults(GTK_TABLE(char_table), char_title, 0, 1,
                                      0, 1);
            gtk_table_attach_defaults(GTK_TABLE(char_table), char_input, 0, 2,
                                      1, 2);
            ...
            gtk_box_pack_start_defaults(GTK_BOX(GTK_DIALOG(char_dialog)->vbox),
                                        char_table);
      ...
      case cal:  // 调用子过程
        s[t + 1] =
            base(i.level_diff, s, b);  // 将父过程基地址入栈，即建立静态链
...
int main(int argc, char* argv[]) {
  ...
  const gchar* symtable_items[9] = {
      "name", "type", "datatype", "level", "address", "size",
  };
  GtkWidget* table;
  ...
  gtk_table_attach(GTK_TABLE(table), halign, 0, 1, 0, 1, GTK_FILL, GTK_FILL, 0,
                   0);
  ...
  gtk_table_attach(GTK_TABLE(table), x0code, 0, 9, 1, 9, GTK_FILL | GTK_EXPAND,
  ...
  gtk_table_attach(GTK_TABLE(table), halign2, 9, 10, 0, 1, GTK_FILL, GTK_FILL,
                   0, 0);
  ...
  gtk_table_attach(GTK_TABLE(table), import, 15, 16, 1, 2, GTK_FILL, GTK_SHRINK,
                   1, 1);
  g_signal_connect(G_OBJECT(import), "clicked", G_CALLBACK(import_onclick),
                   NULL);

  save = gtk_button_new_with_label("Save");
  gtk_widget_set_size_request(save, 50, 30);
  gtk_table_attach(GTK_TABLE(table), save, 15, 16, 2, 3, GTK_FILL, GTK_SHRINK,
                   1, 1);
  ...
  gtk_table_attach(GTK_TABLE(table), halign3, 0, 1, 9, 10, GTK_FILL, GTK_FILL,
                   0, 0);
  ...
  gtk_table_attach(GTK_TABLE(table), halign4, 9, 10, 9, 10, GTK_FILL, GTK_FILL,
                   0, 0);
  ...
  gtk_table_attach(GTK_TABLE(table), run, 0, 1, 15, 16, GTK_FILL, GTK_SHRINK, 1,
                   1);
...
void import_onclick(GtkWidget* widget, gpointer data) {
  ...
  g_signal_connect(G_OBJECT(FileSelection), "response",
                   G_CALLBACK(import_openfile), data);
  ...
  if (response_id == -5) {
    ...
    if (fp != NULL) {
      ...
      while ((ch = fgetc(fp)) != EOF) {
        ...
        GtkTextIter start, end;
        gtk_text_buffer_get_bounds(GTK_TEXT_BUFFER(buffer), &start, &end);
        gtk_text_buffer_insert(GTK_TEXT_BUFFER(buffer), &end, text,
                               strlen(text));
      }
    } else {
      GtkWidget* dialog;
...
void save_openfile(GtkWidget* trigger, gint response_id, gpointer data) {
  if (response_id == -5) {
    ...
    if (fp != NULL) {
      ...
      gtk_text_buffer_get_bounds(GTK_TEXT_BUFFER(buffer), &start, &end);
      text = gtk_text_buffer_get_text(GTK_TEXT_BUFFER(buffer), &start, &end,
                                      FALSE);
      fprintf(fp, "%s", text);
...
void run_onclick(GtkWidget* widget, gpointer data) {
  ...
  if (fp != NULL) {
    ...
    dialog = gtk_message_dialog_new(
        NULL, GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_ERROR, GTK_BUTTONS_OK,
        "Error: fatal error while processing input");
    gtk_window_set_title(GTK_WINDOW(dialog), "Error");
    gtk_dialog_run(GTK_DIALOG(dialog));
    gtk_widget_destroy(dialog);
...
void print_table(GtkWidget* frame) {
  ...
  for (int i = 1; i <= symbolTableTail; i++) {
    ...
    sprintf(name, "%s", symbolTable[i].name);
